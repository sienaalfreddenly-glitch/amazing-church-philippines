-- Three things on notifications:
--   1. Members can clear (delete) their own rows, so a full bell can be
--      emptied without waiting for it to age out.
--   2. Promotion notifications point at the post that was just written, not
--      at the target's profile, so clicking one opens the announcement in
--      the feed like every other post/mention notification.
--   3. The one-line copy on a promotion notification is picked from a small
--      library and stored on the row, so different promotions read
--      differently and the same row keeps its wording between reloads.

-- 1. Delete own notifications.
drop policy if exists "delete own notifications" on public.notifications;
create policy "delete own notifications" on public.notifications
  for delete using (user_id = auth.uid());

-- 2 + 3. Rewrite promote_member.
...
begin
  ...
  -- Fan out. entity_type = 'post' and entity_id = the announcement's id, so
  -- clicking a promoted notification opens the post in the feed. The chosen
  -- one-line notification text is stored in metadata.message so the bell
  -- renders it verbatim.
